...
function testTime(){
    ...
    function testTimeClock() {
        let nowMin = new Date().getMinutes();
        if(nowMin!=preMin){
            console.log("Full minute!");
        }else if(count>=10){
            ...
            num*=2;
            count++;
            setTimeout(testTimeClock,5000);
        }
    }
    return testTimeClock();
}
...
function testMail(telephone,mail) {
    let teleRef=/^1[3,4,5,7,8,9]\d{9}$/;
    let mailRef=/^[a-zA-Z0-9_-]+@[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)+$/;
    let teleAns=teleRef.test(telephone);
    let mailAns=mailRef.test(mail);
    if(teleAns&&mailAns){
        console.log("The telephone is right and the mail is right!");
    }else if(teleAns&&!mailAns){
...
function testSpecialReverse(str) {
    str=str.replace(/\s+/g," ");
    let result = str.trim().split(" ").reverse().join(" ");
    console.log(result);
}
...
function lengthOfLongestSubstring(str) {
    ...
    let j=0;
    for(let i=0; i<len; i++){
        let mid=str.charAt(i);
        if(map.has(mid)){
            j=Math.max(j,map.get(mid)+1);
        }
...
function test() {
    ...
    testMail("18717959733","17300290033@fudan.edu.cn");
    console.log("Test Question3");
    testRedundancy("Is is the iS is cost of of gasoline going up up");
    console.log("Test Question4");
    testKeyBoard("7_This_is_a_test","_hs_s_a_es");
    console.log("Test Question5");
    testSpecialReverse("  hello    world!  ");
    console.log("Test Question6");
    twoSum([1,2,3,4],5);
    console.log("Test Question7");
    lengthOfLongestSubstring("abccccccc");
    console.log("Test Question8");
